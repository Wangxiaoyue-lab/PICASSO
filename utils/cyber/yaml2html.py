...
with open(args.output_file, 'w') as f:
    ...
    f.write('</div>\n')
    f.write('<h1>实验名称:{}</h1>\n'.format(data['实验名称']))
    f.write('<p>实验单位：{}</p >\n'.format(data['实验情况']['实验单位']))
    f.write('<p>实验者：{}</p >\n'.format(data['实验情况']['实验者']))
    f.write('<p>实验日期：{}</p >\n'.format(data['实验情况']['实验日期']))
    f.write('<p>实验平台：{}</p >\n'.format(data['实验情况']['实验平台']))
    f.write('<h1>实验目的</h1>\n')
    #f.write('<p>{}</p >\n'.format(data['实验目的']))
    ...
    f.write('</ul>\n')
    f.write('<h2>实验数据</h2>\n')
    f.write('<p>{}</p >\n'.format(data['实验方法与材料']['实验数据']))
    f.write('<h2>实验软件</h2>\n')
    f.write('<ul>\n')
    for software in data['实验方法与材料']['实验软件']:
        f.write('<li>{}</li>\n'.format(software))
    f.write('</ul>\n')
    f.write('<h2>补充</h2>\n')
    f.write('<p>{}</p >\n'.format(data['实验方法与材料']['补充']))
    f.write('<h1>实验过程</h1>\n')
    f.write('<p>{}</p >\n'.format(data['实验过程']))
    f.write('<h1>实验结果</h1>\n')
    f.write('<p>{}</p >\n'.format(data['实验结果']))
