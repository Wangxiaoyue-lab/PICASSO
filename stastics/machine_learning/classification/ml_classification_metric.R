ml_classification_metric <- function(...) {
    ...
    ml_classification_fscore <- function() {
        next
    }
}

ml_classification_curve <- function(...) {
    ml_classification_curve_roc <- function() {
        next
    }
    ml_classification_curve_pr <- function() {
        next
    }
    ml_classification_curve_ks <- function() {
        next
    }
}

########## summary confusion matrix
# https://github.com/JinmiaoChenLab/Batch-effect-removal-benchmarking/blob/master/Script/simulation/03_DEGs_Fscore/confusion_matrix_Fscore.R
rm(list = ls())
...
main_Fscore <- function(select) {
    ...
    Fscore_list <- lapply(vect_HVG, function(HVG) {
        ...
        df_all_all <- do.call(rbind, lapply(df_all, function(l) {
            l[[1]]
        }))
